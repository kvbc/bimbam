...
var map = MapRoom.new("1x1", 0, 0) # first MapRoom
...
func set_room_as_current_scene (room:MapRoom, entered_from_dir = null):
	...
	get_tree().current_scene.InitRoom(
	...
	if entered_from_dir != null:
		player.global_position = get_tree().current_scene.GetExitPosition(entered_from_dir) + dir_vector(opposite_dir(entered_from_dir)) * 70
	else:
